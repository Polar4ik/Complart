...
var element_cord := {
	...
	ElementList.AND_ELEMENT: Vector2i(3,0),
}
...
var select_block: Vector2i = element_cord[ElementList.SOURCE_BLOCK]
var block_direction := 0

# не лезь оно тебя сожрёт
func _input(_event: InputEvent) -> void:
	list = clamp(list, 0, element_cord.size()-1)
	...
	if Input.is_action_just_pressed("select_item_1"):
		select_block = element_cord[element_cord.keys()[list*4]]
	elif Input.is_action_just_pressed("select_item_2"):
		select_block = element_cord[element_cord.keys()[list*4+1]]
	elif Input.is_action_just_pressed("select_item_3"):
		select_block = element_cord[element_cord.keys()[list*4+2]]
	elif Input.is_action_just_pressed("select_item_4"):
		select_block = element_cord[element_cord.keys()[list*4+3]]
